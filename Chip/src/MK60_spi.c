...
uint32 spi_init(SPIn_e spin, SPIn_PCSn_e pcs, SPI_CFG master,uint32 baud)
{
    ...
    if(spin == SPI0)
    {
        ...
        //进行管脚复用
        port_init(SPI0_SCK , ALT2  );
        port_init(SPI0_SOUT, ALT2  );
        port_init(SPI0_SIN , ALT2  );

        if(pcs & SPIn_PCS0)
            port_init(SPI0_PCS0, ALT2  );

        if(pcs & SPIn_PCS1)
            port_init(SPI0_PCS1, ALT2  );

        if(pcs & SPIn_PCS2)
            port_init(SPI0_PCS2, ALT2  );

        if(pcs & SPIn_PCS3)
            port_init(SPI0_PCS3, ALT2  );

        if(pcs & SPIn_PCS4)
            port_init(SPI0_PCS4, ALT2  );

        if(pcs & SPIn_PCS5)
            port_init(SPI0_PCS5, ALT3  );
    }
    else if(spin == SPI1)
    {
#if defined(MK60DZ10)
    ...
    else if(spin == SPI2)
    {
        ...
        port_init(SPI2_SCK , ALT2  );
        port_init(SPI2_SOUT, ALT2  );
        port_init(SPI2_SIN , ALT2  );

        if(pcs & SPIn_PCS0)
            port_init(SPI2_PCS0, ALT2  );

        if(pcs & SPIn_PCS1)
            port_init(SPI2_PCS1, ALT2  );
    }
    else
    {
